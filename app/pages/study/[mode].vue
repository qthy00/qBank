...
const answerTimer = ref<ReturnType<typeof setInterval> | undefined>(undefined)
const lastAnswerState = ref<string | undefined>(undefined)
...
const isOptionCorrect = (key: string, questionId?: number) => {
  ...
  if (questionId !== undefined) {
    const subQuestion = currentQuestion.value.questionList?.find((item) => item.id === questionId)
    const userAnswer = compositeAnswer.value.find((item) => item.id === questionId)?.answer
    return subQuestion?.answer?.includes(key) && userAnswer?.includes(key)
  }
  return false
}
...
const isOptionIncorrect = (key: string, questionId?: number) => {
  if (!isSubmitted.value || !currentQuestion.value) return false
...
const checkOptionStatus = (key: string, questionId?: number) => {
  if (!isSubmitted.value || !currentQuestion.value) return undefined
  // 获取对应的答案数据
  const targetAnswer =
...
// 子案例题输入处理
const handleSubCaseInput = (subQuestionId: number) => {
  if (isSubmitted.value) return
  const answer = fillAnswers.value[subQuestionId] || ''

  // 更新 compositeAnswer
  const existing = compositeAnswer.value.find((item) => item.id === subQuestionId)
  if (existing) {
...
const submitAnswer = async () => {
  // 检查是否有答案（复合题检查所有子问题，其他检查当前答案）
  const hasAnswer = currentQuestion.value.type === 8
    ? compositeAnswer.value.length > 0 && compositeAnswer.value.every(item => item.answer.length > 0)
    : currentAnswer.value && currentAnswer.value.length > 0

  if (!hasAnswer) {
    useMessage().warning('请先选择或输入答案')
    return
  }
  stopTimer()
  // 判断答案是否正确
  ...
  const answerData = {
    ...
    isMistake: modeType.value == 2,
    toolId: packageId.value,
...
const handleFinish = () => {
  try {
    ...
    navigateTo({name: 'ExerciseResult', params: {toolId: packageId.value}});
  } catch (err) {
